...
if default_output_file then
	if threats then		
		...
		for mode,emit in pairs (threats.Emitters) do
			...
			if threatType then
				jsonEmit = string.format('{ "ID":"%s", "Power":%f, "Azimuth":%f, "Priority":%f, "SignalType":"%s", "Type":"%s" }', emit.ID, emit.Power, emit.Azimuth, emit.Priority, emit.SignalType, emit.Type.level1)		
			else
		...
		jsonThreats = string.format('{ "Mode":%f, "Emitters":%s }', threats.Mode, jsonEmitters)
